...
from promptpaws.verdict import Decision, SAFE_REFUSAL, Signal, combine_signals, default_refusal
...
def screen_output(
    ...
    *,
    ...
    On a block, ``safe_response`` is ``refusal`` (a domain-specific message) rather
    than the original response; it defaults to the shared ``SAFE_REFUSAL``, mirroring
    ``guard()``'s ``refusal`` parameter on the input side.
    """
    signals: list[Signal] = []
    ...
    return ScreenResult(
        ...
        safe_response=(refusal if refusal is not None else default_refusal()) if block else response,
        signals=signals,
    )
