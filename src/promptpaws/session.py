...
class SessionTracker:
    ...
    def __init__(self, max_sessions: int = _MAX_SESSIONS) -> None:
        ...
        # State is mutated (touch/evict/read-modify-write) on every turn, so a
        # threaded server (gunicorn workers share this object) would race the
        # OrderedDict without this. Reentrant because ``record_risk`` calls
        # ``state`` while already holding it.
    ...
    def record_risk(
        ...
        *,
        ...
        """
        # One lock over the whole read-modify-write so concurrent turns of the
        # same session can't interleave and corrupt the running score/window.
        with self._lock:
