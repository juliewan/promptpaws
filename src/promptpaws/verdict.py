...
def default_refusal() -> str:
    """Return the process-level default refusal message.

    ``PROMPTPAWS_REFUSAL`` lets hosted integrations set one domain-specific
    refusal once, while per-call ``refusal=...`` still wins.
    """
    return os.environ.get("PROMPTPAWS_REFUSAL") or SAFE_REFUSAL
