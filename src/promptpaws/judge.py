...
class _LLMJudgeBase:
    ...
    def __init__(
        ...
        *,
        ...
        # The judge is shared across a threaded server's request handlers, so the
        # cache's read/move_to_end/evict is a data race without this.
    ...
    def _call(self, content: str) -> str | None:
        ...
        if self._timeout is None:
            try:
                ...
                return None
        # A per-call executor rather than one shared ``max_workers=1`` pool: a
        # call that hangs past the timeout is abandoned (``shutdown(wait=False)``
        # does not join its thread), so it cannot occupy the sole worker forever
        # and silently brick every later judge call down to rules-only. The cost
        # of a genuinely hung provider is a leaked thread, not a dead layer.
        pool = ThreadPoolExecutor(max_workers=1)
        try:
            future = pool.submit(self._complete, prompt)
...
def openai_complete_from_env() -> Callable[[str], str] | None:
    """Return a stdlib OpenAI chat-completions adapter when env vars are set.

    Reads ``PROMPTPAWS_OPENAI_API_KEY`` or ``OPENAI_API_KEY``. The model defaults
    to ``gpt-5-nano`` and can be changed with ``PROMPTPAWS_JUDGE_MODEL`` or
    ``JUDGE_MODEL``. No vendor SDK dependency is introduced.
    """
    api_key = os.environ.get("PROMPTPAWS_OPENAI_API_KEY") or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        return None
    model = os.environ.get("PROMPTPAWS_JUDGE_MODEL") or os.environ.get("JUDGE_MODEL", "gpt-5-nano")
    request_timeout = float(os.environ.get("PROMPTPAWS_JUDGE_HTTP_TIMEOUT", "30"))
...
def semantic_judge_from_env() -> LLMJudge | None:
    """Build the input semantic judge when OpenAI env vars are configured."""
    complete = openai_complete_from_env()
    if complete is None:
        return None
    timeout = float(os.environ.get("PROMPTPAWS_JUDGE_TIMEOUT", "5"))
    return LLMJudge(complete, timeout=timeout)


def policy_judge_from_env() -> LLMPolicyJudge | None:
    """Build the output policy judge when OpenAI and policy env vars are set."""
    policy = os.environ.get("PROMPTPAWS_POLICY")
    if not policy:
        return None
    complete = openai_complete_from_env()
    if complete is None:
        return None
    timeout = float(os.environ.get("PROMPTPAWS_JUDGE_TIMEOUT", "5"))
    return LLMPolicyJudge(complete, policy=policy, timeout=timeout)
